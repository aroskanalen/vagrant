#!/usr/bin/env bash
DIR=$( cd "$( dirname "${BASH_SOURCE[0]}" )" && pwd )

bold=$(tput bold)
normal=$(tput sgr0)

cd $DIR

if [ -e $DIR/htdocs ]
then
		echo "${bold}Directory $DIR/htdocs already exists. Please remove it before running this script:${normal}"
		echo
		echo "rm -fr $DIR/htdocs"
		echo
		exit
fi

echo "${bold}Cloning repositories${normal}"
mkdir htdocs
cd htdocs

git clone https://github.com/search-node/search_node.git search_node

git clone https://github.com/itk-os2display/docs.git docs
git clone https://github.com/itk-os2display/admin.git admin
git clone https://github.com/itk-os2display/middleware.git middleware

cd ..
